...
import { normalizeToolArgs, harvestGithubToken, extractToolCallsFromText } from '../lib/agentLoop.js'
...
test('text tool calls preserve nested assertions, diagnostic purpose and omitted project paths', () => {
  const call = { tool: 'verify_work', command: 'python3 test.py', label: 'Positive count', kind: 'test', assertions: [{ type: 'json_number', field: 'items', min: 1 }] }
  const parsed = extractToolCallsFromText('Run this:\n```json\n' + JSON.stringify(call) + '\n```')
  assert.deepEqual(parsed, [{ name: call.tool, args: Object.fromEntries(Object.entries(call).filter(([k]) => k !== 'tool')) }])
  const diagnostic = extractToolCallsFromText('{"tool":"execute_command","command":"echo diagnostic","purpose":"diagnostic"}')[0]
  assert.equal(diagnostic.args.purpose, 'diagnostic')
  assert.equal(Object.hasOwn(diagnostic.args, 'projectPath'), false)
  assert.equal(Object.hasOwn(diagnostic.args, 'target'), false)
  const source = 'print({"a": "} and \\\"quoted\\\""})'
  assert.equal(extractToolCallsFromText(JSON.stringify({ tool: 'write_file', path: 'a.py', content: source }))[0].args.content, source)
})
