...
export const AGENT_TOOL_DEFS = [
  ...
  {
    name: 'set_acceptance_criteria', description: 'Record the concrete requirements from the user or project rubric before building. Completion requires a current passing verify_work check for each criterion. Do not invent or weaken user criteria.',
    input_schema: { type: 'object', properties: { criteria: { type: 'array', minItems: 1, maxItems: 12, items: str('One specific requirement, copied or faithfully condensed from the request') } }, required: ['criteria'] },
  },
  {
